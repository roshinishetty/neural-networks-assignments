...
for t=1:iter
    for j=1:size(X,2)
        hist_W(t,j) = W(j) - alpha * (1/(size(X,1)) * (h-Y)' * X(:,j) + regCof * signum(W(j)) );
    end
    ...
    h=X * W;
    J(t) = 1/2 * (sum((h-Y).^2)*1/(size(X,1)) + regCof*sum(W)); 
end    
plot(J)
W
pause;
...
for t=1:iter
 for i=1:size(X,1)
     for j=1:size(X,2)
         hist_W(t,j) = W(j) - alpha*(1/size(X,1)*(h(i)-Y(i))*X(i,j) + regCof * signum(W(j)) ); 
     end    
 end    
 ...
 h = X * W;
 J(t) = 1/2 *( 1/size(X,1) * sum((Y-h).^2) + regCof * sum(W) );
end    
